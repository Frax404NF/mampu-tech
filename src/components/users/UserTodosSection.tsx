    ...
    return (
      <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-5">
        ...
        <div className="flex flex-col items-center py-8 text-center">
          <svg className="w-8 h-8 text-slate-300 mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="9" x2="15" y2="9"/><line x1="9" y1="13" x2="12" y2="13"/></svg>
    ...
    <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {pending.length > 0 && (
            <span className={badgeVariants.amber}>{pending.length} pending</span>
          )}
        </div>
      ...
      <div className={`mt-3 pt-3 border-t border-slate-100 ${activeList.length > PREVIEW_COUNT ? 'block' : 'hidden'}`}>
        <button
          onClick={() => setExpanded((p) => !p)}
